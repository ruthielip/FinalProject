...
const Post = (props) => {
  ...
  useEffect(()=>{
    const getLikes = async ()=>{
      try{
        const likesArray = await axiosInstance.get(`/posts/likes/${post._id}`)
  ...
  const deleteComment = async (e) => {
    try{
      await axiosInstance.put(`/posts/${post._id}/comments`, {commentId: e.target.value});
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`/posts/${post._id}`, {data: {userId: currentUser._id}});
      window.location.reload();
    } catch(err) {
      console.log(err);
    }
  ...
  const handleEdit = async (e) => {
    ...
    try {
      await axiosInstance.put(`/posts/${post._id}`, newDesc);
      window.location.reload();
    } catch(err) {
      console.log(err);
    }
  ...
  const handleComment = async (e) => {
    ...
    try {
      await axiosInstance.put(`/posts/${post._id}/comment`, newComment);
      window.location.reload();
    } catch (err) {
      console.log(err);
    }
